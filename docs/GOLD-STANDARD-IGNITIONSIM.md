...
- Amazon CTAs must use verified direct product pages only: `https://www.amazon.com/dp/ASIN?tag=ignitionsim-20`. Never publish Amazon search-result links (`/s?k=`, `/s?keywords=`, or generic "Check Amazon" destinations). Until Robert explicitly approves another affiliate relationship, **every visible retail-style product card links its product photo, product name, and purchase CTA to the same exact Amazon product page**. If the exact ASIN cannot be verified, retain useful discussion as editorial text but do not render a shopping-style product card. Official, manual, forum, and reviewer links may remain only as clearly labeled research evidence; they never carry buy language, buy styling, `data-buy-link`, or `rel=sponsored`, and they never substitute for an Amazon purchase destination.
...
- A direct-product revenue path. Amazon links use exact verified ASINs with `tag=ignitionsim-20`. If an exact match is unavailable, do not improvise a retail link: keep the product editorial, retain separately labeled evidence where useful, and omit the purchase CTA until an exact Amazon listing or a newly approved affiliate relationship exists.
